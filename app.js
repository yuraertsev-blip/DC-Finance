...
try {
    ...
    db = getFirestore(app);
    auth = getAuth(app);
    console.log('[DC-INIT] Firebase initialized, projectId:', firebaseConfig.projectId);
} catch (e) {
...
function loadState() {
    ...
    unsubscribeSnapshot = onSnapshot(doc(db, 'dc_finance', 'main_state'), (docSnap) => {
        if (docSnap.exists()) {
            ...
            console.log('[DC-LOAD] No document found — pushing defaults');
        ...
        console.error('[DC-LOAD] Firestore listener ERROR:', error);
...
function saveState() {
    ...
    .catch((error) => {
        console.error('[DC-SAVE] ❌ FAILED:', error);
...
function initAuth() {
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            console.log('[DC-AUTH] No user — showing login');
            currentUser = null;
            if (unsubscribeSnapshot) unsubscribeSnapshot();
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        console.error('[DC-INIT] Critical Init Error:', e);
